...
func server(conn *net.UDPConn, conn2 *net.UDPConn, listenport string, service2 string, id int) {
	for {
		...
		if buffer == nil {
			isdown = true
			// mymsg := "0,0,0"
			// RemoteAddr, err := net.ResolveUDPAddr("udp", "localhost:6001")
			// LocalAddr, err := net.ResolveUDPAddr("udp", "localhost:2222")
...
func main() {
	...
	port1, _ := strconv.Atoi(myPort)
	port1 = port1 + 233
	service4 := hostName + ":" + strconv.Itoa(port1)
	service3 := hostName + ":" + "6001"

	RemoteAddr, err := net.ResolveUDPAddr("udp", service)
	LocalAddr, err := net.ResolveUDPAddr("udp", service2)
	conn, err := net.DialUDP("udp", LocalAddr, RemoteAddr)

	RemoteAddr2, err := net.ResolveUDPAddr("udp", service3)
	LocalAddr2, err := net.ResolveUDPAddr("udp", service4)
	conn2, err := net.DialUDP("udp", LocalAddr2, RemoteAddr2)
	...
	for {
		...
		go server(conn, conn2, listenport, service2, id)
		go listen(listenport, readorwrite)
		//go doClientJob(conn, readorwrite, pageId)
		if readorwrite == "read" {
			...
			if isdown == true {
				...
				mymsg2 = strId + "," + strMessage + "," + pageId

			}
			fmt.Println("sending write request to master", mymsg)
			fmt.Println("sending write request to replica", mymsg2)
			buf := []byte(mymsg)
			buf1 := []byte(mymsg2)
...
// func doClientJob(conn *net.UDPConn, readorwrite string, pageId string) {
// 	if readorwrite == "read" {
// 		time.Sleep(time.Second * 3)
